...
func TestProcessActivityRecords(t *testing.T) {
	...
	file, err := os.Open("testdata/Gorge_Gravel_Grinder.fit")
	if err != nil {
	...
	if result.TotalDistance != (150.458960) {
	...
	if result.ElapsedTime != 20772 {
		t.Errorf("Expected elapsed time to be 10800, got %d", result.ElapsedTime)
	}

	if result.StoppedTime != 1695 {
		t.Errorf("Expected stopped time to be 20772, got %d", result.StoppedTime)
	}

	if result.TotalElevationGain != 2530.977051 {
		t.Errorf("Expected TotalElevationGain to be 2530.977051, got %f", result.TotalElevationGain)
	}

	if len(result.PowerResults) != 62 {
	...
	if len(result.CadenceResults) != 62 {
		t.Errorf("Expected power results length to be %d, got %d", len(result.CadenceResults), len(result.CadenceResults))
	}
	...
	if len(result.HeartResults) != 62 {
		t.Errorf("Expected power results length to be %d, got %d", len(result.HeartResults), len(result.HeartResults))
	}
